...
public class TimeAchievementUnlockedSteps {
    ...
    public void checkAchievementExistence(final @Named("id") String id, final @Named("type") AchievementType type, final @Named("event") String event, final @Named("start") String start, final @Named("end") String end) {
        ...
        if(achievement.isPresent()) {
            TimeRangeAchievementBean timeAchievementBean = (TimeRangeAchievementBean) achievement.get();
            assertThat("Achievement is subscribed to event: " + event, timeAchievementBean.getEvent().contains(event), is(true));
            assertThat("Trigger is present for time-range achievement", triggerChecker.isTimeRangeTriggerPresent(timeAchievementBean.getTrigger(), start, end), is(true));
        } else {
